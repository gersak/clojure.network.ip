...
(defprotocol Networkfy
  (networkfy [cidr] [ip-address subnet]))
...
(defrecord Network [^InetAddress ip-address ^Integer subnet]
  ...
  Object
  (toString [this] (str (apply str (rest (str ip-address))) "/" subnet)))
                 ...
                 (if (string? subnet)
                   (do
                     ...
                     (let [subnet (if (string? subnet)
                                    ...
                                    subnet)]
                       (networkfy (str ip-address "/" subnet))))
                   (do
...
(def ipv6-network "FE80:0000:0000:0000:0202:B3FF:FE1E:8329/106")

(def x (networkfy ipv6-network))
